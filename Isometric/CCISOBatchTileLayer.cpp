...
// CCISOBatchTileLayer - atlasIndex and Z
...
void CCISOBatchTileLayer::setupTiles()
{
...
CCSprite* CCISOBatchTileLayer::reusedTileWithRect(CCRect rect)
{
    if (! m_pReusedTile)
    {
...
CCSprite * CCISOBatchTileLayer::tileSpriteAt(const CCPoint& pos)
{
...
unsigned int CCISOBatchTileLayer::atlasIndexForExistantZ(unsigned int z)
{
    int key=z;
...
unsigned int CCISOBatchTileLayer::atlasIndexForNewZ(int z)
{
...
void CCISOBatchTileLayer::setTileGID(unsigned int gid, const CCPoint& pos)
{
...
void CCISOBatchTileLayer::removeChild(CCNode* node, bool cleanup)
{
...
void CCISOBatchTileLayer::removeTileSpriteAt(const CCPoint& pos)
{
    CCAssert(pos.x < m_tLayerSize.width && pos.y < m_tLayerSize.height && pos.x >=0 && pos.y >=0, "TMXLayer: invalid position");
    CCAssert(m_pTiles && m_pAtlasIndexArray, "TMXLayer: the tiles map has been released");
...
void CCISOBatchTileLayer::addQuadFromSprite(CCSprite *sprite, unsigned int index)
{
...
void CCISOBatchTileLayer::addSpriteWithoutQuad(CCSprite*child, unsigned int z, int aTag)
{
...
CCArray* CCISOBatchTileLayer::getTileSets()
{
    return m_pTileSets;
}
